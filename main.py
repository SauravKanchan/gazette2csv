...
with open(sys.argv[1]+'.csv') as csvfile:
    ...
    for row in range(17,len(reader),62):
        for sr in range(0,28,9):
            if '-------' not in reader[row+sr][0]:
                name_ls=reader[row+sr][0].split()
                res_a+=name_ls[0]+", "+ " ".join(name_ls)[len(name_ls[0]):-36] +", "
                a,b,c=get_matrix(reader[row+sr+2][0]+reader[row+sr+2][1][:2],reader[row+sr+3][0]+reader[row+sr+3][1][:2],reader[row+sr+4][0]+reader[row+sr+4][1][:2])
